...
type Persisted = Pick<SettingsState, 'showGitBranchBadges' | 'tabOverflowMode' | 'hotkeyOverrides' | 'mcpSettings'>

function loadSettings(): Persisted {
  if (typeof localStorage === 'undefined') {
    return { showGitBranchBadges: true, tabOverflowMode: 'scroll', hotkeyOverrides: {}, mcpSettings: DEFAULT_MCP_SETTINGS }
  }
  try {
    const raw = localStorage.getItem(SETTINGS_KEY)
    if (!raw) return { showGitBranchBadges: true, tabOverflowMode: 'scroll', hotkeyOverrides: {}, mcpSettings: DEFAULT_MCP_SETTINGS }
...
export const useSettingsStore = create<SettingsState>((set, get) => ({
  ...
  setShowGitBranchBadges: (value) => {
    ...
    const s = get()
    saveSettings({ showGitBranchBadges: s.showGitBranchBadges, tabOverflowMode: s.tabOverflowMode, hotkeyOverrides: s.hotkeyOverrides, mcpSettings: s.mcpSettings })
  },

  setTabOverflowMode: (mode) => {
    set({ tabOverflowMode: mode })
    const s = get()
    saveSettings({ showGitBranchBadges: s.showGitBranchBadges, tabOverflowMode: mode, hotkeyOverrides: s.hotkeyOverrides, mcpSettings: s.mcpSettings })
  },
  ...
  setHotkeyOverride: (id, override) => {
    ...
    set({ hotkeyOverrides })
    const s = get()
    saveSettings({ showGitBranchBadges: s.showGitBranchBadges, tabOverflowMode: s.tabOverflowMode, hotkeyOverrides, mcpSettings: s.mcpSettings })
  },
  ...
  setMcpSettings: (mcpSettings) => {
    set({ mcpSettings })
    const s = get()
    saveSettings({ showGitBranchBadges: s.showGitBranchBadges, tabOverflowMode: s.tabOverflowMode, hotkeyOverrides: s.hotkeyOverrides, mcpSettings })
