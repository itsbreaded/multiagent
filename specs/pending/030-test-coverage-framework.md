...
**Done:**
- Phase 0 — Vitest 4 harness (`vitest.config.ts`, two projects), `tests/setup.renderer.ts`, repo-root `__mocks__/zustand.ts`, `test`/`test:watch`/`test:coverage` scripts, tsconfigs include test files, `.github/workflows/ci.yml` (Windows runner), `.gitignore` covers `coverage/`. 114 tests pass; `npm run typecheck` clean.
...
- Wave 1B: `TranscriptScanner`, `CodexSessionScanner` (session-detection candidate matching), `SessionIndex` FTS query builder, `DeepSearcher` ranking/caps, OSC 633/7 shell-integration parser. These still need extract-and-test passes (determinism notes in the Determinism section apply).
