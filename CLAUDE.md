...
- Codex resume can fork to a new rollout id, but detect that fork via the cwd/time-constrained filesystem scanner ONLY (`_watchForNewCodexSession` with `mode='resume'`, which fires only on a session id different from the resumed one). Do NOT probe (`/status`) on resume: Codex renders `/status` inline into the live transcript and it pollutes the user's real chat history. A resume that does not fork keeps its known id and the resume-mode watcher times out silently (no `detection-failed`).
...
#### PTY session-id probe (`_probeSessionIdViaPty`)

The probe is the primary per-pane detection path (it disambiguates multiple panes sharing one cwd, which the filesystem path deliberately cannot). It runs only for NEW panes (`spawnNew`), never on resume. It feeds raw PTY bytes into an `@xterm/headless` terminal and reads the rendered screen, then injects `/status` and regexes the UUID off the rendered screen. Hard-won constraints — changing any of these has regressed it before:

- **`@xterm/headless` import shape:** it is a webpacked CJS bundle with no `"exports"` map and a dead `"module"` field, so a bundled dynamic `import()` exposes the constructor at `m.default.Terminal`, NOT `m.Terminal`. Resolve as `m.Terminal ?? m.default?.Terminal ?? m.default`. Construct with `allowProposedApi: true`.
- **Read the viewport, not the buffer top.** TUIs emit many startup newlines that push content into scrollback, so the visible screen starts at `buffer.active.baseY`. `readScreen` must read `getLine(baseY + y)` for `y in 0..rows-1` (getLine takes an absolute buffer index). Reading from 0 returns blank scrollback and never sees the prompt/UUID. Join `isWrapped` rows so a soft-wrapped UUID isn't split by the box border.
- **Mirror the real pane geometry during readiness.** The PTY spawns at 80x24 (`createDeferred`) and the renderer fits it shortly after, while the headless buffer is otherwise fixed at 24 rows. Cursor-addressed TUIs (Codex/ratatui) lay out the composer against the real height and pad with trailing newlines; a rows mismatch scrolls the composer out of the headless viewport before it can be read. On each data chunk (until widening) resize the headless buffer to `getPtyLastSize(ptyId) ?? {80,24}`. `PtyManager.getPtyLastSize` exists for this.
- **Widen only for capture, then restore.** On readiness, capture `savedSize = getPtyLastSize(...)`, resize BOTH the PTY and the headless buffer to 240x24 so the `/status` box never truncates the UUID, then restore the PTY to `savedSize` after capture (after the Claude double-Escape). Capturing `savedSize` at readiness (not probe start) matters: by then the renderer has fit, so it is the real size, not the 80x24 fallback.
- **Readiness signals:** Claude `❯` (U+276F), Codex `›` (U+203A). For Codex, do not fire on first `›`; wait for an ~800ms idle period (no PTY output, write queue drained) so the composer is live.
- **Submitting `/status`:** Claude takes `'/status\r'` in one write. Codex must type `'/status'`, wait ~250ms for the slash-command autocomplete to register, THEN send `'\r'` separately — sending them together lets the `\r` land as a composer newline instead of executing. `injectStatus` is idempotent (`state==='injecting'` guard); do NOT re-inject/retry `/status` (each injection pollutes the transcript; the filesystem watcher is the fallback). Claude's `/status` modal is dismissed with two best-effort `\x1b` Escapes ~350ms apart; Codex's is inline (no dismiss).
- Temporary state-machine diagnostics are gated behind the `PROBE_DEBUG` env var (`probeDbg`), zero-cost when unset. Enable with `$env:PROBE_DEBUG=1` before `npm run dev` to trace readiness/inject/capture and dump the rendered screen + raw escaped byte stream on timeout — that raw dump is what diagnoses readiness/geometry regressions.
