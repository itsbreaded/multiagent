...
The terminal stack follows VS Code's integrated-terminal shape, and **shell and agent panes share one pty host**. `node-pty` stays isolated in `src/main/pty/ptyWorker.ts`; `PtyManager` is the single pty-host contract for create/input/resize/kill/data/exit/ready/error. The worker's ready event carries pid, initial cwd, and Windows ConPTY traits. The renderer must apply xterm `windowsPty` and the DA1 `\x1b[?61;4c` response only after `pty:ready`, not at xterm construction time.

Shell vs agent differ only by a per-PTY **flow-control policy**, not by a separate host/worker. `PtyManager.createShell` creates a PTY with `flowControl:false` and tracks it in `directIds`; `handlers.ts` then relays that output **directly** (`sendDirectPtyOutput`, `seq=0`) so the renderer writes it synchronously to xterm (`if (seq === 0) terminal.write(data)`). Agent panes (created via `SessionSpawner` → `createDeferred`, default `flowControl:true`) keep the coalesce + sequence/ack flow-control for high-volume CLI output. **Do not route shell output through the coalesce/ack path** — that repeatedly dropped short no-scroll output such as `git pull -> Already up to date.` on the target Windows machine. The seq=0 direct-write contract is the regression guard; preserve it. (The earlier `ShellPtyHost`/`shellWorker` dual-stack and the `bareterm`/`shellterm` scaffolding are removed — do not reintroduce them.) `PtyManager.buildEnv` takes an `'agent' | 'shell'` profile so plain shell panes get a clean environment without the `CLAUDE_CODE_*` vars; `TERM_PROGRAM=vscode` is set for both (shell integration depends on it).
