...
Normal shell panes use `ShellPtyHost` + `shellWorker.ts`, a direct worker relay based on the proven in-app Bare Term path from spec 012. Shell panes still use normal pane IDs, `pty:*` IPC, `pty:ready`, `pty:cwd`, window routing, and close/transfer plumbing.

**Both shell and agent panes now relay PTY output directly** (`sendDirectPtyOutput`, `seq=0`, synchronous `terminal.write` in the renderer). The old coalesce + sequence/ack + pause/resume flow control was removed (agents were the last user of it): in practice agent output is handled fine by node-pty + xterm directly, the same way a heavy interactive PowerShell session is. Do NOT reintroduce coalescing/ack/backpressure for the shell path — it is what dropped short no-scroll output (`git pull -> Already up to date.`) on the target Windows machine. The only buffering left is `pendingDirectOutput` in `handlers.ts`, used solely to hold output while a pty briefly has no routable window (e.g. mid cross-window move); it flushes via `flushDirectOutput` on (re)route.

**The dedicated `shellWorker` *process* is load-bearing — not just the direct data policy.** A unification attempt (commit `27ec130`, reverted) kept the exact seq=0 direct relay and identical ConPTY spawn config but moved shell PTYs into the shared `ptyWorker` process; the no-scroll drop returned immediately. This matches spec 012's finding that a direct send through the `ptyManager`/`ptyWorker` path still dropped. So shell and agent panes must keep **separate worker processes**: shell on `shellWorker`, agents on `ptyWorker`. Do not merge them into one worker. You may share host *contracts*/types and renderer code, but not the worker process.
