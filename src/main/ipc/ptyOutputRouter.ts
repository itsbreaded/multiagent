...
import { parseOsc7, parseShellIntegrationCwd, unterminatedOscTail } from '../pty/shellIntegration'
import type { WindowManager } from '../window/WindowManager'

// spec 050: the optional scraper seam. Kept as a structural type so the router stays
// decoupled from the scraper module (and so tests can stub it). When absent the data
// listener skips the feed call -- zero overhead when the feature is off or absent.
export interface PaneDataScraper {
  feed: (ptyId: string, data: string) => void
  release: (ptyId: string) => void
}

export const PTY_ROUTE_RETRY_MS = 50
export function createPtyOutputRouter(deps: { ptyManager: PtyManager; windowManager: Pick<WindowManager, 'sendToWindowForPty' | 'unroutePty'>; onCommandComplete?: (cwd: string) => void; scraper?: PaneDataScraper }) {
  ...
  function releasePty(id: string): void { releaseBuffer(id);lastCwd.delete(id);oscTails.delete(id);deps.scraper?.release(id) }
  deps.ptyManager.on('data',(id:string,data:string)=>{send(id,data);deps.scraper?.feed(id,data);const scan=(oscTails.get(id)??'')+data;oscTails.set(id,unterminatedOscTail(scan));if(scan.includes('\x1b]633;D')){const cwd=lastCwd.get(id);if(cwd)deps.onCommandComplete?.(cwd)}if(scan.includes('\x1b]7;')||scan.includes('\x1b]633;P;Cwd=')){const cwd=parseShellIntegrationCwd(scan)??parseOsc7(scan);if(cwd&&lastCwd.get(id)!==cwd){lastCwd.set(id,cwd);deps.windowManager.sendToWindowForPty(id,'pty:cwd',id,cwd)}}})
